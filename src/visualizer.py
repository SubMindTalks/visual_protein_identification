...
class ProteinVisualizer:
    ...
    def process_pdb(self, pdb_path: str, metadata: Optional[dict] = None) -> None:
        """Process a single PDB file and generate all visualization types.

        Args:
            pdb_path: Path to PDB file.
            metadata: Optional metadata for the protein.
    ...
    def _render_view(self, pdb_content: str, pdb_name: str, rep_type: str, orientation: str) -> None:
        ...
        try:
            ...
            viewer.setView([0, 0, 0, *rotation])
            viewer.zoomTo()

            # Render and save image
            filename = f"{pdb_name}_{rep_type}_{orientation}.tiff"
            ...
            logger.error(f"Failed to render view {rep_type}-{orientation} for {pdb_name}: {e}")
    ...
    def get_center_of_mass(structure: Structure) -> np.ndarray:
        ...
        for atom in structure.get_atoms():
            ...
            masses.append(atom.mass if atom.mass else 1.0)
